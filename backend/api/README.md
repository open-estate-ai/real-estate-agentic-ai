...
```bash
cat > test-backend-api-lambda.sh << 'EOF'
#!/bin/bash

# Configuration
AWS_REGION="${AWS_REGION:-us-east-1}"
AWS_ACCOUNT_ID="${AWS_ACCOUNT_ID:-756375699536}"
ENVIRONMENT="${ENVIRONMENT:-dev}"
LAMBDA_FUNCTION_NAME="${ENVIRONMENT}-open-estate-ai-backend-api"
LAMBDA_FUNCTION_ARN="arn:aws:lambda:${AWS_REGION}:${AWS_ACCOUNT_ID}:function:${LAMBDA_FUNCTION_NAME}"

case "$1" in
  health)
    echo "Testing GET /api/ endpoint..."
    PAYLOAD="{\"version\":\"2.0\",\"routeKey\":\"GET /api/\",\"rawPath\":\"/api/\",\"headers\":{\"accept\":\"application/json\"},\"requestContext\":{\"accountId\":\"$AWS_ACCOUNT_ID\",\"http\":{\"method\":\"GET\",\"path\":\"/api/\",\"sourceIp\":\"127.0.0.1\"}},\"isBase64Encoded\":false}"
    ;;
  ...
  analyze)
    QUERY="${2:-Find 3BHK apartments in Noida}"
    USER_ID="${3:-test-user}"
    echo "Testing POST /api/analyze endpoint..."
    echo "Query: $QUERY"
    echo "User: $USER_ID"
    BODY="{\\\"query\\\":\\\"$QUERY\\\",\\\"user_id\\\":\\\"$USER_ID\\\"}"
    PAYLOAD="{\"version\":\"2.0\",\"routeKey\":\"POST /api/analyze\",\"rawPath\":\"/api/analyze\",\"headers\":{\"content-type\":\"application/json\"},\"requestContext\":{\"accountId\":\"$AWS_ACCOUNT_ID\",\"http\":{\"method\":\"POST\",\"path\":\"/api/analyze\",\"sourceIp\":\"127.0.0.1\"}},\"body\":\"$BODY\",\"isBase64Encoded\":false}"
    ;;
  ...
  job)
    ...
    JOB_ID="$2"
    echo "Testing GET /api/jobs/$JOB_ID endpoint..."
    PAYLOAD="{\"version\":\"2.0\",\"routeKey\":\"GET /api/jobs/{job_id}\",\"rawPath\":\"/api/jobs/$JOB_ID\",\"pathParameters\":{\"job_id\":\"$JOB_ID\"},\"headers\":{\"accept\":\"application/json\"},\"requestContext\":{\"accountId\":\"$AWS_ACCOUNT_ID\",\"http\":{\"method\":\"GET\",\"path\":\"/api/jobs/$JOB_ID\",\"sourceIp\":\"127.0.0.1\"}},\"isBase64Encoded\":false}"
    ;;
  ...
  *)
    echo "Usage: $0 {health|health-db|analyze [query] [user_id]|job <job_id>|logs}"
    echo ""
    ...
    echo "  $0 health-db"
    echo "  $0 analyze"
    echo "  $0 analyze 'Find apartments in Delhi' user-456"
    echo "  $0 job job-abc12345"
    echo "  $0 logs"
    exit 1
    ;;
esac

echo "Invoking: $LAMBDA_FUNCTION_ARN"
aws lambda invoke \
  --function-name "$LAMBDA_FUNCTION_ARN" \
  --payload "$PAYLOAD" \
  --cli-binary-format raw-in-base64-out \
...
chmod +x test-backend-api-lambda.sh
```

### Usage Examples

```bash
# Set your environment (optional, defaults shown)
export AWS_REGION="us-east-1"
export AWS_ACCOUNT_ID="756375699536"
export ENVIRONMENT="dev"

# Basic health check
./test-backend-api-lambda.sh health

# Health check with DB status
./test-backend-api-lambda.sh health-db

# Analyze a query (default)
./test-backend-api-lambda.sh analyze

# Analyze with custom query
./test-backend-api-lambda.sh analyze "Find 2BHK in Mumbai" "user-789"

# Get job status
./test-backend-api-lambda.sh job job-abc12345

# View logs
./test-backend-api-lambda.sh logs
```
